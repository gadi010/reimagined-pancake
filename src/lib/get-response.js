...
export const getResponse = async (code) => {
  ...
  try {
    ...
    const tokenText = await tokenResp.text(); // read raw response first
    try {
      ...
      console.error("GitHub token response not JSON:", tokenText);
      throw new Error("Failed to parse GitHub token response");
    }

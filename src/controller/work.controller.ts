...
import { IWork, IWorkAssignee } from "../types/controllers.js";
import { IWorkDTO } from "../types/common.js";


export const createWork = async (req: Request, res: Response) => {
  try {
    const { title, description, assignee, type }: IWorkDTO = req.body;
...
export const updateWork = async (req: Request, res: Response) => {
  ...
  const { title, description, type, status, assignee }: IWorkDTO = req.body;
...
export const getWorks = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const works: IWork[] = await prisma.work.findMany({
      where: {
        projectId: parseInt(projectId),
        ...(userId && {
          assignees: {
    ...
    const formattedWorks = works.map(work => ({
      ...work,
      assignees: work.assignees?.map((a: IWorkAssignee) => a.user) ?? [],
    }));
    return res.send({
      link: {
